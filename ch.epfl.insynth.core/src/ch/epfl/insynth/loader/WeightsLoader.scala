...
import ch.epfl.insynth.env.Weight
  ...
  object WeightsLoader {
    ...
    private final val THIS_WEIGHT = 5.
    private final val LITERAL_WEIGHT = 200.    
    private final val LOCAL_WEIGHT = (5., 10.)
    private final val MNT_DECL_WEIGHT = 20.
    private final val NESTED_DECL_WEIGHT = 20.
    private final val PACKAGE_DECL_WEIGHT = 25.    
    private final val COERCTION_WEIGHT = 10.
    private final val ABSTRACT_DECL_WEIGHT = 1.
    private final val IMPORTED_DECL_WEIGHT = 1000.
    private final val PREDEF_DECL_WEIGHT = 1000.
    private final val SUPERTYPE_DECL_WEIGHT = 1000.
    ...
    def loadLocalWeights(locals:List[Declaration]){
      ...
      x => 
	   if (localWeight < LOCAL_WEIGHT._2) localWeight+=1.
